...
class Block:
    ...
    def rectangles_to_draw(self) -> List[Tuple[Tuple[int, int, int],
        ...
        results = []
        if self.children == []:
            results.extend([(self.colour, self.position,
                             (self.size, self.size), 0),
                            (FRAME_COLOUR, self.position,
                             (self.size, self.size), 3)])
        else:
            for child in self.children:
                results.extend(child.rectangles_to_draw())

        if self.highlighted is True:
            results.append((HIGHLIGHT_COLOUR, self.position,
    ...
    def smash(self) -> bool:
        ...
        if self.level != self.max_depth and self.level != 0:
            self.children = []
            for _ in range(4):
                block = random_init(self.level + 1, self.max_depth)
                block.parent = self
    ...
    def update_block_locations(self, top_left: Tuple[int, int],
        ...
        if len(self.children) == 4:
            ...
            self.children[0].position = (top_left[0] + child_size, top_left[1])
            self.children[1].position = (top_left[0], top_left[1])
            self.children[2].position = (top_left[0], top_left[1] + child_size)
            self.children[3].position = (top_left[0] + child_size, top_left[1] +
                                         child_size)
    ...
    def get_selected_block(self, location: Tuple[int, int], level: int) \
        ...
        - 0 <= level <= max_depth
        """
        if level == self.level or len(self.children) == 0:
            return self

        else:
            if self.children[1].position[0] <= location[0] < \
                    self.children[0].position[0]:
                if self.children[1].position[1] <= location[1] < \
                ...
                if self.children[0].position[1] <= location[1] < \
                        self.children[3].position[1]:
    ...
    def flatten(self) -> List[List[Tuple[int, int, int]]]:
        ...
        if len(self.children) == 0:
            for i in range(2 ** (self.max_depth - self.level)):
                column = []
                for _ in range(2 ** (self.max_depth - self.level)):
                    column.append(self.colour)
            ...
            for i in range(len(child0)):
                result.append(child0[i] + child3[i])
...
def random_init(level: int, max_depth: int) -> 'Block':
    ...
    Precondition:
        level <= max_depth
    """
    parent_block = Block(level)
    parent_block.max_depth = max_depth
    if level == max_depth:
        parent_block.colour = random.choice(COLOUR_LIST)
        return parent_block
    elif random.random() >= math.exp(-0.25 * level):
        parent_block.colour = random.choice(COLOUR_LIST)
        return parent_block
    else:
        children = []
        for _ in range(4):
            child = random_init(level + 1, max_depth)
