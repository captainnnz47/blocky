...
class RandomPlayer(Player):
    """A random player.
        ...
        A RandomPlayer is a computer player that, as the name implies,
        chooses moves randomly. Random players have no limit on their smashes.
    ...
    def make_move(self, board: Block) -> int:
        ...
        Return 0 upon successful completion of a move, and 1 upon a QUIT event.
        """
        block = pick_random_block(board)
        block.highlighted = True
        self.renderer.draw(board, self.id)
        pygame.time.wait(TIME_DELAY)

        random_move = random.choice(range(5))
...
class SmartPlayer(Player):
    """ A smart player.
        ...
        A smart player is a computer player that chooses moves more
        intelligently: It generates a set of random moves and, for each,
        ...
        === Attributes ===
        difficulty: int
            indicates how difficult it is to play against it.
    ...
    def make_move(self, board: Block) -> int:
        ...
        Return 0 upon successful completion of a move, and 1 upon a QUIT event.
        """
        num_moves = moves_to_compare(self.difficulty)
        best_move = [0, pick_random_block(board)]
        best_score = 0

        for _ in range(num_moves):
            random_move = random.choice(range(4))
            block = pick_random_block(board)
        ...
        best_move[1].highlighted = True
        self.renderer.draw(board, self.id)
        pygame.time.wait(TIME_DELAY)

        if best_move[0] == 0:
            best_move[1].rotate(1)
        elif best_move[0] == 1:
            best_move[1].rotate(3)
        elif best_move[0] == 2:
            best_move[1].swap(0)
        else:
            best_move[1].swap(1)

        best_move[1].highlighted = False
        self.renderer.draw(board, self.id)

        return 0


def pick_random_block(board: Block) -> Block:
    """Randomly choose a block."""
    random_level = random.choice(range(board.max_depth))
    random_x = random.choice(range(board.size))
    random_y = random.choice(range(board.size))

    return board.get_selected_block((random_x, random_y), random_level)


def moves_to_compare(difficulty) -> int:
    """Identify the moves for each difficulty level"""
    if difficulty == 0:
        return 5
    elif difficulty == 1:
        return 10
